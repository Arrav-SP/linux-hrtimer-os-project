...
DATA_DIR = "data"
OUT_DIR = os.path.join(DATA_DIR, "native_analysis", "figures")
...
for filename in glob.glob(os.path.join(DATA_DIR, "*.csv")):
    ...
    if not basename.startswith("native_"):
        continue
...
ax.set_title(
    "Native Fedora: mean timing error with standard-deviation error bars"
)
...
ax.set_title(
    "Native Fedora: median timing error across requested delays"
)
...
ax.set_title(
    "Native Fedora: 99th-percentile timing error"
)
...
ax.set_title(
    "Native Fedora: maximum observed timing error"
)
...
ax.set_title(
    "Native Fedora: timing-error distribution for 1 ms requested delay"
)
